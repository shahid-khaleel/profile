...
| [observability](https://github.com/shahid-khaleel/observability) | A 5-service payment platform (gateway/auth/transaction/ledger/fraud-check) instrumented across all five observability planes at once on Istio: STRICT mTLS, Prometheus/Grafana RED metrics, OpenTelemetry → Jaeger tracing, and an EFK pipeline that splits routine logs from a separate compliance audit-event index. Every layer verified with real captured evidence — one trace with correct parent-child spans across all 5 services, `connection_security_policy=mutual_tls` on every hop, live audit documents in Elasticsearch — not just deployed and assumed working. |
